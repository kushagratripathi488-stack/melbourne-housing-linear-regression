...
for column in data_column: 
    if column !='Price' :
        data=fill_missing_with_mean(data,data_column.index(column))
        data=winsorization(data,data_column.index(column))
        data=mean_normalization(data,data_column.index(column))
#Train Test Split
target_idx=data_column.index('Price')
X_train,y_train,X_test,y_test=train_test_split(data,target_idx,0.2)
...
final_cost=cost_func(X_train+X_test,y_train+y_test,weight,bias)
#Evaluation Criteria 
